                        ...
                        <SidebarMenuItem key={item.name}>
                            <SidebarMenuButton asChild className={`cursor-pointer mb-2 p-3! py-5! ${isActive(item.url) ? "bg-teal-500/10 hover:bg-teal-500/15" : ""}`}>
                                <Link href={item.url} className="flex items-center">
                                    <item.icon className={`size-6! stroke-2! pt-1 mb-0.5! ${isActive(item.url) ? "text-teal-600" : "text-zinc-500"}`} />
                                    <span className={`text-sm font-semibold ${isActive(item.url) ? "text-teal-500" : "text-zinc-500"}`}>{item.name}</span>
                                </Link>
                            </SidebarMenuButton>
                        </SidebarMenuItem>
                    ))}
